...
const localFormatsFile = path.resolve(process.cwd(), '.fancygit/formats.json');
const globalFormatsFile = path.resolve(os.homedir(), '.fancygit/config/formats.json');
...
function formatList() {
    ...
    if (Object.keys(formats).length === 0) {
        console.log(chalk.yellow("No formats available. Use 'fancygit format add <name>' to add a new format."));
        return;
    }
