...
// Get the current directory of this file, regardless of how it's imported (global or local)
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Path to defaultFormatter.json in config directory
const defaultFormatterFile = path.resolve(__dirname, '../config/defaultFormatter.json');
...
async function commit(path, options = {}) {
    ...
    try {
        ...
        while (true) {
            ...
            // Step 9: Publish npm package (if configured)
            if (settings.triggerNpm) {
                spinner.stop();
                await publishNpm(spinner);
            }
...
// Helper function to handle npm publish
async function publishNpm(spinner) {
    try {
        const publishVersion = await confirm({
            message: 'Would you like to publish the package to npm?',
            default: false
        });
        if (publishVersion) {
            spinner.start('Publishing npm package...');
            execSync(`npm publish`);
            spinner.succeed('NPM package published successfully!');
        }
    } catch (error) {
        if (error.name === 'ExitPromptError') {
            console.log(chalk.bold.red("\nGoodbye my friend 👋"));
